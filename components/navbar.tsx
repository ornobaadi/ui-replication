...
function ChargeflowLogo({ className, hideText }: { className?: string; hideText?: boolean }) {
    return (
        ...
        >
            {/* Icon mark — the real chargeflow logo */}
            <svg
                width="31"
                height="24"
                viewBox="0 0 31 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-auto shrink-0 text-white"
            >
                <path
                    fillRule="evenodd"
                    clipRule="evenodd"
            ...
            {/* Wordmark SVG — hidden when navbar is shrunk */}
            <div
                className={cn(
...
export function Navbar() {
    ...
    return (
        ...
        >
            ...
            <nav
                className={cn(
                    'grid grid-cols-[1fr_auto_1fr] w-9/10 items-center',
            ...
            >
                {/* Col 1 — Logo (left-aligned, shrinks width but stays in col) */}
                <div className="flex items-center">
                    <ChargeflowLogo hideText={scrolled && !mobileOpen} />
                </div>

                {/* Col 2 — Nav links (always centred; pill fades in default state) */}
                <ul className={cn(
                    ...
                    {NAV_LINKS.map((link) => (
                        <li key={link.label}>
                            <a
                                href={link.href}
                                className={cn(
                                    'block px-3.5 py-1.5 text-xs font-semibold tracking-tight rounded-full',
                ...
                {/* Col 3 — CTAs (right-aligned) + mobile burger */}
                <div className="flex items-center justify-end gap-4">
                    ...
                    >
                        {/* SIGN UP label — visible when not scrolled */}
                        <span className={cn(
                        ...
                        {/* SCHEDULE A DEMO label — visible when scrolled */}
                        <span className={cn(
                            'whitespace-nowrap transition-all duration-500 ease-out overflow-hidden',
            ...
            >
                <div className="flex h-full flex-col justify-between gap-4 p-6">
                    ...
                    <div className="flex flex-col gap-3 items-center">
                        <a
                            href="#"
                            className="flex w-full items-center justify-center gap-1.5 rounded-full border border-white/20 py-3 text-sm font-semibold text-white"
                        >
                            SIGN IN <ArrowUpRight className="h-4 w-4" />
                        </a>
                        <a
                            href="#"
                            className="flex w-full items-center justify-center gap-1.5 rounded-full bg-blue-600 py-3 text-sm font-bold text-white"
                        >
